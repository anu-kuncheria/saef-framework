...
class MobilitiResults:
    ...
    def read_file(self):
        file = pd.read_csv(self.path, sep='\t', header=None, names=self.colnames)
...
class MobilitiAnalysis:
    ...
    def resultscity_filter_len(self, df):
        ...
        df_city  = df_sub.merge(self.city_links, left_on = 'link_id', right_on = 'LINK_ID', how = 'left')
        return df_city
    ...
    def vmt_city(self):
        ...
        neighborhood_flow_df = self.city_flow[self.city_flow['new_classi']== 'Neighbourhood Residential street']
        neighborhood_vmt = self.vmt(neighborhood_flow_df)
        neighborhood_networklen = np.round(neighborhood_flow_df['LENGTH(meters)'].sum()*0.000621371, decimals = 0)
        total_networklen = np.round(self.city_flow['LENGTH(meters)'].sum()*0.000621371, decimals = 0)
        prop = (neighborhood_vmt/total_vmt)/(neighborhood_networklen/total_networklen)
        
        return total_vmt, self.round_two_decimals(prop)
    ...
    def vhd_city(self):
        ...
        if total_vhd > 0:
            neighborhood_flow_df = self.city_flow[self.city_flow['new_classi']== 'Neighbourhood Residential street']
            neighborhood_speed_df = self.city_speed[self.city_speed['new_classi']== 'Neighbourhood Residential street']
            neighborhood_vhd = self.vhd(neighborhood_flow_df, neighborhood_speed_df)
            neighborhood_networklen = np.round(neighborhood_flow_df['LENGTH(meters)'].sum()*0.000621371, decimals = 0)
            total_networklen = np.round(self.city_flow['LENGTH(meters)'].sum()*0.000621371, decimals = 0)
            prop = (neighborhood_vhd/total_vhd)/(neighborhood_networklen/total_networklen)
        else:     #adjusting for 0 vhd in load calculations
            prop = 0 

        return total_vhd, self.round_two_decimals(prop)
    ...
    def congested8am_miles(self, flow):
        ...
        congested_miles = flow[flow['v_c8am']>=1]['LENGTH(meters)'].sum()/1000*0.621371
        return self.round_two_decimals(congested_miles)


    def SPF_segments(self, flow):
        alpha_highways = {1:0, 2:-7.09,3:-7.09,4:-5.78,5:-6.49,6:-6.49,7:-6.49,8:-10.75} # 1 lnae is ramps, so added as 0
        beta_highways = {1:0, 2:0.98, 3:0.98, 4:0.82, 5:0.89, 6:0.89, 7:0.89, 8:1.24}
        ...
        fc2_flow['SPF'] = (fc2_flow['LENGTH(meters)']*0.000621371) * (np.exp(fc2_flow['alpha'])) * (fc2_flow['ADT']**fc2_flow['beta']) 
