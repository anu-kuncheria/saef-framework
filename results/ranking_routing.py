...
def ranking_worse(df, sim=['B60', "UET", 'SOT', 'SOF'], compare1='B60', compare2='SOF'):
    allcols = ['vmt_', 'vhd_', 'fuel_', 'vmtload_nrs_',
               'vhdload_nrs_', 'cong_miles_', 'highway_acc_']
    rankdf = df[['city']]
    city_idx = []
    metric_maxval = []
    for col in allcols:
    ...
    for k, v in city_max.items():
        ...
        for eachv in city_max[k]:
            if eachv[-3:] == compare1:
                ...
                compare2_counter += 1
        city_worse_df.loc[len(city_worse_df), :] = [
            city_name, compare1_counter, compare2_counter]
        city_worse_df[[compare1, compare2]] = city_worse_df[[
            compare1, compare2]].astype(int)

    return rankdf, city_worse_df


if __name__ == "__main__":
    trafficmetrics_cities = pd.read_csv(os.path.join(processed_path_metrics,
                                                     "cities_mobility_metrics.csv"))
    rank_df, city_worse_df = ranking_worse(data=trafficmetrics_cities, sim=[
                                           'B60', "UET", 'SOT', 'SOF'], compare1='B60', compare2='SOF')
    print(rank_df)
    print(city_worse_df)
