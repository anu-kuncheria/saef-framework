...
for sim in simulations_run:
    ...
    dir_names = [f for f in os.listdir(
        processed_path) if not f.startswith('.')]

    for d in dir_names:
        cityname = d
        print(f" Starting city {cityname} ....")
        citylinks_path = os.path.join(
            processed_path, d, f'citylinks_{cityname}.geojson')

        traff = trm.MobilitiAnalysis(
            citylinks_path, flow_path=sim[1], speed_path=sim[2], fuel_path=sim[3])

        total_vmt, nrs_vmtload, nrs_vmt = traff.vmt_city()
        total_vhd, nrs_vhdload, nrs_vhd = traff.vhd_city()
        ...
        data.append(
            {'city': d,
             ...
             f'vmt_nrs_{sim[0]}': nrs_vmt,
             f'vhd_nrs_{sim[0]}': nrs_vhd,
             f'cong_miles_{sim[0]}': congested_8am_miles,
             f'highway_acc_{sim[0]}': estimated_highway_accidents})

    allsim_data.append(data)

b60_df, uet_df, sot_df, sof_df = pd.DataFrame(allsim_data[0]), pd.DataFrame(
    allsim_data[1]), pd.DataFrame(allsim_data[2]), pd.DataFrame(allsim_data[3])
traffic_df = reduce(lambda x, y: pd.merge(x, y, on='city', how='outer'), [
                    b60_df, uet_df, sot_df, sof_df])
print(traffic_df.shape)
print(traffic_df.head(2))
print(traffic_df.columns)
traffic_df.to_csv(os.path.join(processed_path_metrics,
                  "cities_mobility_metrics.csv"), index=False)
